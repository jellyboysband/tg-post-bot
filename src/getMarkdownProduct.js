...
const product2markdown = async (product) => {
    ...
    const longUrl = `${process.env.DEEP_LINK}${qs.stringify({ sub: process.env.APP_ID, to: product.url })}`
    let shortUrl, hash;
    try {
        const short = await wretch(process.env.SHORTER || 'http://localhost:3001/api/v1/shorten').post({ long_url: longUrl, c_new: true }).json()
